...
export interface TabsProps {
  ...
  /** Distributes the tabs evenly across the full width instead of sizing them
   * to their content inside a horizontal scroller. */
  stretch?: boolean;
}
 ...
 */
export const Tabs = ({ items, activeTabId, onTabPress, stretch = false }: TabsProps) => {
  ...
  const renderItems = () =>
    items.map((item, index) => {
      ...
      return (
        <View
          key={item.id}
          style={tailwind.style('flex-row items-center z-10', stretch && 'flex-1')}
          onLayout={e => {
          ...
          <Pressable
            ...
            style={({ pressed }) =>
              tailwind.style(
                'px-4 py-1.5 justify-center items-center',
                stretch && 'flex-1',
                pressed && 'opacity-70',
              )
            }>
  ...
  if (stretch) {
    return (
      <View style={tailwind.style('w-full')}>
        <View
          style={tailwind.style('bg-gray-50 rounded-lg overflow-hidden flex-row items-center')}
